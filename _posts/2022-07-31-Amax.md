...
\end{align*}
where $D_\eta^*$ denotes the geometric mean of eigenvalues of the matrix $\eta\Lambda \delta_{ij} + a^{ij}$. By the dominated convergence theorem and $u\in \Sob{2}{n}(\Omega_\varepsilon)$, we get 
\[	\sup_{\Omega_\varepsilon} u \leq \varepsilon + \sup_{\partial \Omega} u^++C\norm{f/D^*}{ L_{n}(\Omega)}.   \]

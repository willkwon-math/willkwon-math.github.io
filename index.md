...
where $\Omega$ is a bounded Lipschitz domain in $\mathbb{R}^n$, $n\geq 2$. Assuming that $\mathbf{b}\in L^{n,\infty}(\Omega)^n$ has non-negative weak divergence in $\Omega$, we establish existence and uniqueness of weak solution in  of the problem (b) when $A$ is bounded and uniformly elliptic. As an application, we prove unique solvability of weak solution $u$ in $\bigcap_{q<2} W^{1,q}_0(\Omega)$ for the problem (a) for every $f\in \bigcap_{q<2} W^{−1,q}(\Omega)$.
</div>
</details>
